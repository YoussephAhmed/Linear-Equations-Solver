...
double determinant(int n, float mat[100][100])
{
...
double get_determinant()
{
    ...
    return ( determinant(eqs_size ,mat) ) ;
}
...
my_find(input_equations[i],operators , positions);
...
while(positions.empty()!= true)
{
    ...
    if(flag)
    {
        ...
        positions.pop();
        if(input_equations[i].substr(v_start,1) == "-")
            negative = true;
        flag = false;
    }
    ...
    else
    {
        ...
        positions.pop();
          ...
          if(input_equations[i].substr(v_start-1,1) == "-")
            negative = true;
    }


    string s_var = input_equations[i].substr(v_start, v_end-v_start);
...
string s_rhs = input_equations[i].substr(v_end,input_equations[i].length()-v_end);

if(s_rhs.substr(0,1) =="=")
    s_rhs= s_rhs.substr(1,s_rhs.length()-1);

float sum;
sum = atof(s_rhs.c_str());
